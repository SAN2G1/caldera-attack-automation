...
BASE_DIR = "/home/swlab/ttps2_attack_server"
...
@app.route("/agents/auto")
def agents_auto():
    """
    /agents/auto 로 접속하면 IP 기반으로 적절한 agent로 리다이렉트.
    Ability에서 URL 고정하기 쉬움.
    """
    client_ip = get_client_ip()
    agent_name = pick_agent_for_ip(client_ip)
    log_event(f"[AGENT_AUTO] ip={client_ip} -> {agent_name}")
    return redirect(f"/agents/{agent_name}", code=302)
